        ...
        if(this.generalSettings['change_image'] != "0" && $("amasty_zoom") && $("amasty_gallery")) {
            ...
            jQuery("#amasty_gallery a").bind(self.generalSettings['change_image'], function(e) {  
                ...
                if (jQuery(this).attr('data-video')) {
                    jQuery('#amasty_zoom').replaceWith(jQuery(this).attr('data-play-video'));
                } else if (jQuery(this).attr('data-image')) {
                    jQuery('#amasty_gallery a').removeClass('active');
                    jQuery(this).addClass('active');
                    if (jQuery('#filofax-video')) {
                        jQuery('#filofax-video').replaceWith(jQuery('#filovid').attr('data-replace'));
                    }
                    
                    var ez =   jQuery('#amasty_zoom').data('elevateZoom');
                    if (typeof ez == 'undefined') {
                        jQuery("#amasty_zoom").elevateZoom(self.zoomSettings);
                        var ez =   jQuery('#amasty_zoom').data('elevateZoom');
                    }
