...
class Filofax_Upsells_Block_Banner extends Mage_Catalog_Block_Product_Abstract
{
    ...
    public function getValidRules()
    {
        ...
        $validBannersRule = array();
        $storeConfig = Mage::getStoreConfig('ambannerslite/banners/single');
        if ($storeConfig === '1' && count($validRules) > 1) {
            foreach ($validRules as $validRule) {
                $ruleData = $validRule->getData();
                if ($this->_checkForBannersData($ruleData)) {
                    $validBannersRule[] = $validRule;

                    return $validBannersRule;
                }
            }
        }
     ...
     * @return bool
     */
    protected function _checkForBannersData($ruleData)
    {
        $isValid = false;
        if ($ruleData['ambannerslite_top_banner_enable'] != 0
            || $ruleData['ambannerslite_after_name_banner_enable'] != 0) {
            $isValid =  true;
        }
