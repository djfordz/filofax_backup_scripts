...
class Filofax_Upsells_Block_Adminhtml_Promo_Quote_Edit_Tab_BannerNew
    ...
    implements Mage_Adminhtml_Block_Widget_Tab_Interface
{
    ...
    protected function _prepareForm()
    {
        ...
        $product = $layout->createBlock('ambannerslite/adminhtml_promo_quote_edit_tab_products_product');
        ...
        $this->_initFieldSets($form, $model, $upsellModel, $product, $serializer, $category);
        ...
        if ($rule) {
            ...
            $upsellModel->loadByRuleId($ruleId);
        }
    ...
    protected function _initFieldSets($form, $model, $upsellModel, $product, $serializer, $category)
    {
        $this->_addDisplayOptions($form, $model, $product, $serializer, $category);
        $this->_addYearlyUpsellsBanner($form, $upsellModel);
        $this->_addTopBanner($form, $model);
        $this->_addAfterBanner($form, $model);
        $this->_addProductLabel($form, $model);
    }

    protected function _addDisplayOptions($form, $model, $product, $serializer, $category)
    {
        ...
        $fldSet->addField(
            ...
            'hidden',
            array(
                'after_element_html' => $product->toHtml() . $serializer->toHtml(),
            )
        );
    ...
    protected function _addYearlyUpsellsBanner($form, $model)
    {
        ...
        $fldSet->addField(
            'filoupsell_product_banner_img',
            'image',
            array(
                'label' => $this->__('Image'),
                'name' => 'filoupsell_product_banner_img',
            )
        );

        $fldSet->addField(
            'filoupsell_product_banner_alt',
            'text',
            array(
                'label' => $this->__('Alt'),
                'name' => 'filoupsell_product_banner_alt',
            )
        );

        $fldSet->addField(
            'filoupsell_product_banner_hover_text',
            'text',
            array(
                'label' => $this->__('On Hover Text'),
                'name' => 'filoupsell_banner_hover_text',
            )
        );

        $fldSet->addField(
            'filoupsell_product_banner_link',
            'text',
            array(
                'label' => $this->__('Link'),
                'name' => 'filoupsell_product_banner_link',
            )
        );

        $fldSet->addField(
            'filoupsell_product_banner_gift_images_enable',
            'select',
            array(
                'name' => 'filoupsell_product_banner_gift_images_enable',
        ...
        $fldSet->addField(
            'filoupsell_product_banner_description',
            'editor',
            array(
                'name' => 'filoupsell_product_banner_description',
    ...
    protected function _addProductLabel($form, $model)
    {
        ...
        $fldSet->addField(
            ...
            array(
                'after_element_html' =>
                    $this->__(
                        'Get attractive and various labels with <a href="%s" target="_blank">"Product Labels"</a> 
extension.', "https://amasty.com/product-labels.html"
                    ),
            )
        );
